...
class Config:
    ...
    # Upload settings
    MAX_CONTENT_LENGTH: int = int(os.environ.get("MAX_UPLOAD_MB", "16")) * 1024 * 1024
    ...
    # ML model
    MODEL_PATH: str = os.environ.get("MODEL_PATH", os.path.join("models", "food_freshness.pt"))
    MODEL_VERSION: str = os.environ.get("MODEL_VERSION", "1.0.0")
    MODEL_INPUT_SIZE: int = 224

    # Confidence thresholds
    CONFIDENCE_HIGH: float = float(os.environ.get("CONFIDENCE_HIGH", "0.70"))
    CONFIDENCE_MEDIUM: float = float(os.environ.get("CONFIDENCE_MEDIUM", "0.50"))

    # Rate limiting
    RATELIMIT_DEFAULT: str = os.environ.get("RATELIMIT_DEFAULT", "200 per day;50 per hour")
    RATELIMIT_PREDICT: str = os.environ.get("RATELIMIT_PREDICT", "30 per hour;5 per minute")
